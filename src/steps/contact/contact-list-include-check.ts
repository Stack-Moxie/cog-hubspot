  ...
  async executeStep(step: Step) {
    ...
    try {
      while (!contactFound && hasMore) {
        const response = await this.client.getContactListById(listId, vidOffset);
        console.log(response.contacts.map(c => c.vid));
        contactFound = response.contacts.map(c => c.vid).find(c => +c === +contactId);
