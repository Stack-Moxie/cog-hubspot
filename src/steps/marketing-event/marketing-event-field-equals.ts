...
export class MarketingEventFieldEquals extends BaseStep implements StepInterface {
  ...
  protected stepExpression: string = 'the (?<field>[a-zA-Z0-9_-]+) field on hubspot marketing external event id (?<id>.+\@.+\..+) should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)?';
  ...
  async executeStep(step: Step) {
    ...
    try {
      ...
      // Hubspot only accepts utc so you have to convert the date to local time again
      const dateTokenFormat = /\d{4}-\d{2}-\d{2}(?:.?\d{2}:\d{2}:\d{2}.{4})Z?/;
      if (dateTokenFormat.test(actual)) {
        actual = moment.utc(actual).toDate().toISOString();
      }
