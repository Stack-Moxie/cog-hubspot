...
import { Step } from '../../../src/steps/ticket/ticket-field-equals';

chai.use(sinonChai);

describe('TicketFieldEquals', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getStepId()).to.equal('TicketFieldEquals');
      expect(stepDef.getName()).to.equal('Check a field on a HubSpot ticket');
      expect(stepDef.getExpression()).to.equal('the (?<field>[a-zA-Z0-9_-]+) field on hubspot ticket (?<id>.+) should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)?');
  ...
  describe('ExecuteStep', () => {
    describe('Expected Parameters', () => {
      it('should call getTicketById with expected id', async () => {
        ...
        expect(clientWrapperStub.getTicketById).to.have.been.calledWith(expectedId);
      });
    });
    ...
    describe('Expected field not found', () => {
      beforeEach(() => {
        ...
        clientWrapperStub.getTicketById.returns(Promise.resolve({
          properties: {
            hs_pipeline: 0,
            hs_pipeline_stage: 1,
    ...
    describe('Ticket expected field value equals expectation', () => {
      beforeEach(() => {
        protoStep.setData(Struct.fromJavaScript({
          id: '123123',
    ...
    describe('Ticket expected field value not equal expectation', () => {
      beforeEach(() => {
        protoStep.setData(Struct.fromJavaScript({
          id: '123123',
    ...
    describe('Error occurred', () => {
      beforeEach(() => {
        ...
        clientWrapperStub.getTicketById.throws('error');
      });
