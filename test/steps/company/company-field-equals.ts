...
import { Step } from '../../../src/steps/company/company-field-equals';

chai.use(sinonChai);

describe('CompanyFieldEquals', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getStepId()).to.equal('CompanyFieldEquals');
      expect(stepDef.getName()).to.equal('Check a field on a HubSpot Company');
      expect(stepDef.getExpression()).to.equal('the (?<field>[a-zA-Z0-9_-]+) field on hubspot company (?<id>.+) should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)?');
  ...
  describe('ExecuteStep', () => {
    describe('Expected Parameters', () => {
      it('should call getCompanyById with expected id', async () => {
        ...
        expect(clientWrapperStub.getCompanyById).to.have.been.calledWith(expectedId);
      });
    });
    ...
    describe('Expected field not found', () => {
      beforeEach(() => {
        ...
        clientWrapperStub.getCompanyById.resolves({
          companyId: 3123123,
          properties: {
            subject: {
              value: 'anySubject'
    ...
    describe('Company expected field value equals expectation', () => {
      beforeEach(() => {
        protoStep.setData(Struct.fromJavaScript({
          id: '123123',
    ...
    describe('Company expected field value not equal expectation', () => {
      beforeEach(() => {
        protoStep.setData(Struct.fromJavaScript({
          companyId: '123123',
    ...
    describe('Error occurred', () => {
      beforeEach(() => {
        ...
        clientWrapperStub.getCompanyById.throws('error');
      });
